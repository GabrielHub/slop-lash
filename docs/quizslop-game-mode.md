...
Server phases may include short reveal and dispute substates, but those must not
...
After all selected question groups have appeared, open one bounded dispute
window and persist its boundary-active initiator roster. Each eligible player may
successfully initiate at most one dispute per game and must select one revealed
question plus one reason:
...
question while the window remains open.
...
If several distinct questions are challenged, batch them into one vote phase.
At vote opening, freeze a new boundary-active voter roster.
Every eligible voter, including initiators and people who did not receive that
question, submits `UPHOLD` or `VOID` for each ballot.
...
- All ballots share one deadline and settle atomically.

Question and call outcomes remain provisional until the dispute window and any
batched vote close. An unchallenged question is valid; a challenged question is
...
| -------------------------- | ---------------------------: | --------------------------------------------------------- |
| Final House Choice vote    |                   12 seconds | Abstain; resolve plurality and frozen rank                |
| House Choice result        |                    3 seconds | Advance automatically                                     |
| Topic reveal               |                    8 seconds | Advance automatically                                     |
| Call Slop                  |                   10 seconds | Hold                                                      |
| Call Slop reveal           |                    3 seconds | Advance automatically                                     |
| Private answer             |                   25 seconds | Accountable player is incorrect                           |
| Question reveal            | 6 seconds per selected group | Advance to next group                                     |
| Dispute window             |                    8 seconds | Do not initiate                                           |
| Batched dispute vote       |                   12 seconds | Abstain; non-majority upholds                             |
| Round results              |                    8 seconds | Advance automatically                                     |
...
- all-resolved submission phases may still advance automatically;
...
| Question reveal   | Selected question groups, answers, sources, players, and provisional deltas                        |
...
- initiate at most one successful dispute and vote on batched disputes;
...
`QUESTION_REVEAL` also stores the current bounded reveal ordinal. Do not create
one database phase name per question group.
...
| From                | Condition                                                                                                        | To                                        |
| ------------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `LOBBY_SETUP`       | Atomic start succeeds                                                                                            | `TOPIC_REVEAL` for warm-up                |
...
| `TOPIC_REVEAL`      | Reveal beat closes                                                                                               | `SLOP_CALL`                               |
...
| `ANSWER`            | No eligible assignment exists because every player was exempt                                                    | `DISPUTE_WINDOW`                          |
| `QUESTION_REVEAL`   | More groups remain                                                                                               | `QUESTION_REVEAL` at next ordinal         |
| `QUESTION_REVEAL`   | Last group closes                                                                                                | `DISPUTE_WINDOW`                          |
| `DISPUTE_WINDOW`    | No ballot opens                                                                                                  | Settle, then `ROUND_RESULTS`              |
| `DISPUTE_WINDOW`    | One or more ballots open                                                                                         | `DISPUTE_VOTE`                            |
| `DISPUTE_VOTE`      | Everyone resolves or deadline/host close applies abstentions                                                     | Settle, then `ROUND_RESULTS`              |
...
| `QUESTION_REVEAL`, `DISPUTE_WINDOW`, `DISPUTE_VOTE`, `ROUND_RESULTS`, `CONTINUITY_GRACE` | `ROUND_RESULTS` |
...
3. Build one interactive four-player fixture covering:
   ...
   - private questions and grouped shared reveals;
   - one accountable timeout and one pre-answer disconnect exemption;
   - two distinct challenges resolved in one batched dispute vote;
...
2. Implement pure functions for hidden-tier updates, catalog availability,
   ...
   assignment, Call Slop, batched disputes, settlement, and final ties.
...
4. Implement server-authoritative setup, roster/deck freeze, phases, deadlines,
   final vote, calls, assignments, answers, grouped reveals, batched disputes,
...
- timers on and host-controlled timers off;
...
| Risk                                                       | Guardrail                                                                                                               |
| ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| The game has too many steps                                | Four-beat round, no visible tiers, no warm-up vote, one optional call, exceptional batched disputes                     |

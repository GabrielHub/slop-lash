...
const PATTERN = String.raw`\b(?:text|bg|border|from|to|via|ring|stroke|fill)-(?:white|black|gray|zinc|slate|neutral|stone|red|orange|amber|yellow|lime|green|emerald|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)(?:-|\/|\b)`;
...
const ALLOWLIST = [
  ...
  /src[\\/]+app[\\/]+game[\\/]+\[code\][\\/]+voting-reveal\.tsx:.*text-white/,
];
...
if (violations.length > 0) {
  ...
  console.error(
    "If a white/black utility is truly intentional, add a narrow allowlist entry in scripts/check-theme-colors.mjs.",
  );
  process.exit(1);
}

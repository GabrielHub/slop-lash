...
describe("QuizSlop view redaction", () => {
  ...
  test("matches lobby readiness to the connected 2-8 player start roster", async () => {
    const backend = createTestBackend();
    const room = await createQuizslopRoom(backend, { joinerNames: ["Bea", "Cody"] });
    const presence = createPresenceController(backend);
    const [host, bea, cody] = room.players;
    if (!host || !bea || !cody) throw new Error("Missing test players");

    await chooseTopic(backend, host, CATALOG_TOPIC_IDS[0]!);
    await chooseTopic(backend, bea, CATALOG_TOPIC_IDS[1]!);
    await presence.heartbeatAll([host, bea]);

    // A disconnected player without a topic is not in the frozen start roster
    // and therefore does not block two connected, ready players.
    let stage = await stageViewOf(backend, room.host);
    let controller = await controllerViewOf(backend, room.host);
    expect(stage.lobby).toMatchObject({ canStart: true, minPlayers: 2, maxPlayers: 8 });
    expect(controller.lobby).toMatchObject({ everyoneReady: true, canStart: true });
    expect(stage.lobby?.statuses.find((status) => status.playerId === cody.playerId)).toMatchObject(
      { connected: false, state: "NEEDS_TOPIC" },
    );

    // Once that player is connected, their missing topic blocks the exact same
    // roster the start mutation would freeze.
    await presence.heartbeat(cody);
    stage = await stageViewOf(backend, room.host);
    controller = await controllerViewOf(backend, room.host);
    expect(stage.lobby?.canStart).toBe(false);
    expect(controller.lobby).toMatchObject({ everyoneReady: false, canStart: false });

    await chooseTopic(backend, cody, CATALOG_TOPIC_IDS[2]!);
    stage = await stageViewOf(backend, room.host);
    expect(stage.lobby?.canStart).toBe(true);

    // Ready players who disconnect no longer satisfy the two-player minimum.
    await presence.disconnect(bea);
    await presence.disconnect(cody);
    stage = await stageViewOf(backend, room.host);
    controller = await controllerViewOf(backend, room.host);
    expect(stage.lobby?.canStart).toBe(false);
    expect(controller.lobby).toMatchObject({ everyoneReady: false, canStart: false });
  ...
  test("exposes pack status and withholds topic offers until a pack is playable", async () => {
    const backend = createTestBackend();
    const room = await createQuizslopRoom(backend, { joinerNames: ["Bea"] });
    const presence = createPresenceController(backend);
    ...
    const setPackStatus = async (packStatus: Doc<"quizSlopState">["packStatus"]) => {
      await backend.run(async (ctx) => {
        ...
        await ctx.db.patch("quizSlopState", state._id, { packStatus });
      });
    };

    for (const packStatus of ["PENDING", "GENERATING", "FAILED"] as const) {
      await setPackStatus(packStatus);
      const stage = await stageViewOf(backend, room.host);
      const controller = await controllerViewOf(backend, room.host);
      expect(stage.lobby).toMatchObject({ packStatus, canStart: false });
      expect(controller.lobby).toMatchObject({
        packStatus,
        offers: [],
        everyoneReady: true,
        canStart: false,
    ...
    for (const packStatus of ["CATALOG_READY", "READY", "FALLBACK"] as const) {
      await setPackStatus(packStatus);
      const stage = await stageViewOf(backend, room.host);
      const controller = await controllerViewOf(backend, room.host);
      expect(stage.lobby).toMatchObject({ packStatus, canStart: true });
      expect(controller.lobby).toMatchObject({
        packStatus,
        everyoneReady: true,
        canStart: true,
      });
      expect(controller.lobby?.offers).toHaveLength(3);
    }
  });
  ...
  test("redacts questions, keys, and tiers before reveal and shares sources safely after", async () => {
    ...
    expect(await hostAdvance(backend, room.host)).toBe("QUESTION_REVEAL");

...
export function StageLobby({
  ...
  const packBuilding = lobby.packStatus === "PENDING" || lobby.packStatus === "GENERATING";
  const packFailed = lobby.packStatus === "FAILED";
  const topicChoicesOpen = !packBuilding && !packFailed;
  const connected = lobby.statuses.filter((status) => status.connected);
  const connectedWaitingForTopic = connected.filter((status) => status.state !== "READY").length;
  const startLabel = starting
    ? "Starting..."
    : packBuilding
      ? "Building question pack..."
      : packFailed
        ? "Question pack unavailable"
        : connected.length < lobby.minPlayers
          ? `Need ${lobby.minPlayers - connected.length} more connected`
          : connected.length > lobby.maxPlayers
            ? `Limit ${lobby.maxPlayers} connected players`
            : connectedWaitingForTopic > 0
              ? `Waiting for ${connectedWaitingForTopic} topic${connectedWaitingForTopic === 1 ? "" : "s"}`
              : "Start the show";
  ...
  return (
    ...
    >
        ...
        {lobby.statuses.map((status) => (
          ...
          >
            <span className="flex min-w-0 items-center gap-2">
              ...
              {!status.connected && (
                <span
                  className="shrink-0 rounded px-1.5 py-0.5 font-mono text-sm font-bold uppercase"
            ...
            <span
              className="inline-flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 font-mono text-sm font-bold uppercase tracking-wider"
              style={
                topicChoicesOpen && status.state === "READY"
      ...
      {packBuilding ? (
        <motion.output
          variants={fadeInUp}
          className="block max-w-xl rounded-2xl border px-5 py-4 text-center text-base"
          style={{
        ...
        >
          {lobby.packStatus === "PENDING"
            ? "The fresh question pack is queued. Players can keep joining while it starts."
            : "Fresh questions are being written and checked. Topic choices will open automatically when the pack is ready."}
        </motion.output>
      ) : packFailed ? (
        <motion.p
          variants={fadeInUp}
          role="alert"
          className="max-w-xl rounded-2xl border px-5 py-4 text-center text-base font-medium"
          style={{
        ...
        >
          The fresh pack failed and no reviewed catalog fallback was loaded. This room cannot start;
          create a new QuizSlop room with Reviewed Catalog.
        </motion.p>
      ) : (
        <motion.div variants={fadeInUp} className="flex max-w-xl flex-col gap-2 text-center">
          ...
          <p className="text-sm" style={{ color: "var(--qs-ink-dim)" }}>
            Everyone secretly locks in one topic they actually know. Nobody sees it until their
            round.
          </p>
          <p className="text-sm" style={{ color: "var(--qs-ink-dim)" }}>
            Only connected players count when the host starts; offline players do not block the
            room.
          </p>
        </motion.div>
      )}
...
export function StageHouseVote({
  ...
  reveal: boolean;
  winnerTopic: QuizslopViewCurrentTopic | null;
}) {
    ...
    <section className="mx-auto flex w-full max-w-4xl flex-col items-center gap-8">
      ...
      >
        {slate.map((entry) => {
          ...
          const isWinner = reveal && winnerTopic !== null && entry.topicId === winnerTopic.topicId;
            ...
            >
              ...
              {reveal && houseVote.voteCounts !== null && (
                <p className="flex items-center gap-2">
                  ...
                  {isWinner && (
                    <span
                      className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 font-mono text-sm font-bold uppercase tracking-wider"
                      style={{ background: "var(--qs-marquee)", color: "var(--qs-accent-ink)" }}
                    >
...
export function StageSlopCall({
  ...
  return (
    <section className="mx-auto flex w-full max-w-2xl flex-col items-center gap-6 text-center">
      ...
      <p className="font-mono text-sm font-bold" style={{ color: "var(--qs-punch)" }}>
        Miss: +150 · Right: -150
      </p>
      <ProgressMeter
        label="Calls are in..."
...
export function StageAnswer({
  ...
  return (
    <section className="mx-auto flex w-full max-w-2xl flex-col items-center gap-6 text-center">
      ...
      <p className="max-w-md text-sm sm:text-base" style={{ color: "var(--qs-ink-dim)" }}>
        Questions are live on player phones. Choose one answer and stay quiet until every answer
        locks.
      </p>
      <ProgressMeter

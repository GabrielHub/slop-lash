...
/* ─── Dispute window: low-key challenge flow ─── */
  ...
  return (
    <section className="flex flex-col gap-4">
      <ControllerReveal
        ...
        revealOrdinal={revealGroups.length - 1}
        revealTotal={revealGroups.length}
      ...
      {!dispute.canInitiate && (
        <WaitingCard text="Dispute window open. You've already used your one challenge this game." />
      )}
    </section>
  );
}
...
export function ControllerDisputeVote({
  ...
  return (
    <section className="flex flex-col gap-4">
      <SectionTitle
        title="Rule on the challenges"
        hint="Uphold keeps the key. Void throws the question out."
      />
      {ballots.map((ballot) => {
        const myVote = myVoteByDispute.get(ballot.disputeId) ?? null;
        const busy = busyDisputeId === ballot.disputeId;
        return (
          <div
            key={ballot.disputeId}
            className="flex flex-col gap-3 rounded-2xl border p-4"
    ...
    <section className="flex flex-col gap-4">
      ...
      >
        ...
        <div className="mt-1 flex items-center gap-2">
          ...
          <TokenChips remaining={me.tokensRemaining} size={16} />
        </div>
      </div>
    </section>
  );
    ...
    <section className="flex flex-col gap-4">
      {abandoned ? (
        ...
        <SectionTitle
          ...
          hint={mine && !mine.winner ? `You finished #${myIndex + 1}.` : undefined}
        />
      )}
        ...
        {final.standings.map((standing, index) => {
          ...
          return (
            ...
            >
              ...
              <span
                className="flex shrink-0 items-baseline gap-2 font-mono text-[11px] tabular-nums"
                style={{ color: "var(--qs-ink-dim)" }}
              >
        ...
        <ul className="flex flex-col gap-2">
          {final.awards.map((award) => (
            ...
            >
              ...
              <p className="font-mono text-[11px]" style={{ color: "var(--qs-ink-dim)" }}>
                {award.stat}
              </p>
            </li>
          ))}

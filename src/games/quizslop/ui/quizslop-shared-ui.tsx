...
export function RoundKindBadge({ kind }: { kind: QuizslopRoundKind }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 font-mono text-[11px] font-bold uppercase tracking-[0.2em]"
      style={palette}
    >
      <BoltIcon size={12} />
...
export function PointValueTag({ pointValue, finale }: { pointValue: number; finale: boolean }) {
  return (
    <span
      className="inline-flex items-center rounded-full border px-3 py-1 font-mono text-[11px] font-bold uppercase tracking-[0.2em]"
      style={
        finale
...
export function CategoryTag({ category }: { category: QuizslopCategory }) {
  return (
    <span
      className="inline-flex items-center rounded-full px-2.5 py-0.5 font-mono text-[10px] font-bold uppercase tracking-[0.18em]"
...
export function ResultBadge({
  ...
  if (voided) {
    return (
      <span
        className="inline-flex items-center gap-1 rounded-md px-2 py-0.5 font-mono text-[11px] font-bold uppercase tracking-wider"
        style={{ color: "var(--qs-ink-dim)", background: "var(--qs-raised)" }}
      >
        <CrossIcon size={12} /> Voided
      </span>
    );
  }
  if (timedOut) {
  ...
  return correct ? (
    <span
      className="inline-flex items-center gap-1 rounded-md px-2 py-0.5 font-mono text-[11px] font-bold uppercase tracking-wider"
      style={{ color: "var(--qs-win)", background: "var(--qs-win-soft)" }}
    >
...
export function RulingBadge({ ruling }: { ruling: QuizslopQuestionRuling }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1 rounded-md border px-2 py-0.5 font-mono text-[11px] font-bold uppercase tracking-wider"
      style={
        voided
        ...
        return (
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              {!entry.connected && (
                <span
                  className="shrink-0 rounded px-1 font-mono text-[9px] font-bold uppercase"
                  style={{ color: "var(--qs-fail)", background: "var(--qs-fail-soft)" }}
                >
                  offline
                </span>
            ...
            <span className="flex items-baseline justify-between gap-2">
              ...
              <TokenChips remaining={entry.tokensRemaining} size={13} />
            </span>
            <span
        ...
        <span className="flex shrink-0 items-center gap-1.5">
          ...
          {group.players.map((player) => (
            <span key={player.playerId} className="inline-flex items-center gap-1">
              <span className="font-display text-xs font-bold" style={{ color: "var(--qs-ink)" }}>
                {player.name}
              </span>
              <ResultBadge
          ...
          {group.choices.map((choice, index) => {
            ...
            return (
              ...
              >
                ...
                {isKey && (
                  <span
                    className="flex shrink-0 items-center gap-1 font-mono text-[10px] font-bold uppercase tracking-wider"
        ...
        {group.players.map((player) => {
          ...
          return (
            ...
            >
              ...
              <span className="font-mono text-xs" style={{ color: "var(--qs-ink-dim)" }}>
...
export function TopicCard({
  ...
  return (
    ...
    >
      ...
      <span className="text-xs leading-snug" style={{ color: "var(--qs-ink-dim)" }}>
        {topic.scope}
      </span>
    </span>
  );
